...
const navItems = [
    ...
    { href: "/blogs", label: "Blogs", key: "blog" },
...
const serviceFooterLinks = [
    { href: "portfolio#social", label: "Social Media Marketing" },
    { href: "portfolio#seo", label: "SEO & PPC" },
    { href: "portfolio#brand", label: "Brand Identity" },
    { href: "portfolio#media", label: "Media Production" },
    { href: "portfolio#web", label: "Web Development" },
    { href: "portfolio#remote", label: "Dedicated Remote Staff" },
];
...
const socialLinks = [
    ...
    {
        href: "https://x.com/squadtechsol",
        label: "X",
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="currentColor"><path d="M453.2 112L523.8 112L369.6 288.2L551 528L409 528L297.7 382.6L170.5 528L99.8 528L264.7 339.5L90.8 112L236.4 112L336.9 244.9L453.2 112zM428.4 485.8L467.5 485.8L215.1 152L173.1 152L428.4 485.8z" /></svg>`,
    },
];
...
function navLinksMarkup(activeKey) {
    return navItems
        .map(
            (item) =>
                `<a href="${item.href}" class="nav-link${item.key === activeKey ? " is-active" : ""}">${item.label}</a>`,
        )
        .join("");
}
    ...
    <header class="site-header">
      ...
      <div id="mobile-menu" class="mobile-menu md:hidden">
        <div class="mobile-links">
          ...
          <a href="/contact" class="primary-button">Start a Project</a>
          </div>
        </div>
      </div>
    </header>
...
function renderFooter(activeKey) {
    ...
    const footerLinks = links
        ...
        .join("");
    const serviceLinks = serviceFooterLinks
        .map((item) => `<a href="${item.href}">${item.label}</a>`)
        .join("");
    const socials = socialLinks
        .map(
            (item) =>
                `<a href="${item.href}" class="social-link" target="_blank" rel="noreferrer" aria-label="${item.label}">${item.icon}</a>`,
        )
        .join("");
